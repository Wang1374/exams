    ...
    <el-table :data="tableData" border stripe style="width: 99%">
      ...
      <el-table-column label="操作" :width="cloumWidth.cz">
        <template #default="scope">
          <el-button size="mini" v-if="scope.row.examStatus===0" type="success" @click="handleEdit(scope.row.id,scope.row.examStatus)" icon="el-icon-s-promotion">去考试</el-button>
          <el-tag v-else-if="scope.row.examStatus===1" size="medium" type="info">无操作</el-tag>
          <el-button size="mini" v-else type="info" icon="el-icon-view" @click="examQueryGrade(scope.row.userGradeId,scope.row.examStatus)">查看成绩</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div style="margin: 10px 0">
...
export default {
  ...
  methods: {
    ...
    handleEdit(id, examStatus) {
      this.$router.push("/testPaperStu");
      sessionStorage.setItem(
        "StudentExamId",
        this.$encryption(JSON.stringify(id))
      );
      sessionStorage.setItem("examId", id);
      sessionStorage.setItem("StudentExamStatus", examStatus);
    },
    examQueryGrade(id) {
      this.$router.push("/testPaperStuQueryGrade");
      sessionStorage.setItem(
        "StudentQueryGradeInfo",
        this.$encryption(JSON.stringify(id))
      );
    },
    isToken(res) {
