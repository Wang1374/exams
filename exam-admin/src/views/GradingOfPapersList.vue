<template>
  <div style="padding:10px;" class="kd" v-loading="loading">
    ...
    <el-table size="small" :data="tableData" border stripe style="width: 99%">
      <el-table-column :width="cloumWidth.id" label="序号" type="index" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="id" label="ID" v-if="false">
      </el-table-column>
      <el-table-column prop="userName" label="姓名" :show-overflow-tooltip="true" :width="cloumWidth.userName">
      </el-table-column>
      ...
      <el-table-column prop="gradeAuto" label="自动评分" :width="cloumWidth.gradeAuto">
      </el-table-column>
      ...
      <el-table-column prop="examStatus" label="试卷状态" :width="cloumWidth.examStatus">
        <template v-slot="scope">
          <el-tag v-if="scope.row.examStatus===1" size="medium" type="danger">待批改</el-tag>
          <el-tag v-else size="medium" type="success">已完成</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" :width="cloumWidth.cz">
        <template #default="scope">
          <el-button size="mini" v-if="scope.row.examStatus===1" type="primary" @click="handleEdit(scope.row.id,scope.row.examStatus)" icon="el-icon-s-promotion">批改</el-button>
          <el-button size="mini" v-else type="info" icon="el-icon-view" @click="handleEdit(scope.row.id,scope.row.examStatus)">复查</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div style="margin: 10px 0">
...
export default {
  ...
  methods: {
    ...
    defaultTableWith() {
      ...
      let kd = [50, 70, 80, 80, 50, 70, 50, 80, 80, 80, 170, 170, 82, 98];
    ...
    handleEdit(id, examStatus) {
      this.$router.push("/markTestPaper");
      sessionStorage.setItem(
        "markTestPaperId",
        this.$encryption(JSON.stringify(id))
      );
      sessionStorage.setItem("markTestPaperStatus", examStatus);
    },
    isToken(res) {
      if (res === "") {
