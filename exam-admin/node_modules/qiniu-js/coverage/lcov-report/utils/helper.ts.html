...
<head>
    <title>Code coverage report for utils/helper.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">utils</a> helper.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">58.23% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>92/158</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                <span class="strong">59.33% </span>
                <span class="quiet">Lines</span>
                <span class='fraction'>89/150</span>
            </div>
...
<a name='L343'></a><a href='#L343'>343</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">5x</span>
...
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (chunkByteSize &gt; file.size) {
...
<span class="cstat-no" title="statement not covered" >    logger.warn(new QiniuError(</span>
...
&nbsp;
  ...
  try {
    ...
    // 本地信息已被破坏，直接删除
<span class="cstat-no" title="statement not covered" >    removeLocalFileInfo(localKey, logger)</span>
<span class="cstat-no" title="statement not covered" >    logger.warn(new QiniuError(</span>
...
<span class="cstat-no" title="statement not covered" >    return new window.ActiveXObject('Microsoft.XMLHTTP')</span>
  }
&nbsp;
...
export function readAsArrayBuffer(data: Blob): Promise&lt;ArrayBuffer&gt; {
  return new Promise((resolve, reject) =&gt; {
    ...
    reader.onload = (evt: ProgressEvent&lt;FileReader&gt;) =&gt; {
      <span class="missing-if-branch" title="else path not taken" >E</span>if (evt.target) {
...
&nbsp;
    reader.onerror = <span class="fstat-no" title="function not covered" >() =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      reject(new QiniuError(</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (xhr.status === 0) {</span>
        // 发生 0 基本都是网络错误，常见的比如跨域、断网、host 解析失败、系统拦截等等
<span class="cstat-no" title="statement not covered" >        reject(new QiniuNetworkError('network error.', reqId))</span>
<span class="cstat-no" title="statement not covered" >        return</span>
      }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >          data = JSON.parse(responseText)</span>
        } catch {
          // 无需处理该错误、可能拿到非 json 格式的响应是预期的
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        reject(new QiniuRequestError(xhr.status, reqId, message, data))</span>
<span class="cstat-no" title="statement not covered" >        return</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      try {</span>
...
export function getDomainFromUrl(url: string | undefined): string {
  if (url &amp;&amp; url.match) {
    ...
    return groups ? groups[1] : <span class="branch-1 cbranch-no" title="branch not covered" >''</span>
  }
&nbsp;
  return ''
}
...
export function getPutPolicy(token: string): PutPolicy {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!token) <span class="cstat-no" title="statement not covered" >throw new QiniuError(QiniuErrorName.InvalidToken, 'invalid token.')</span>
&nbsp;
  const segments = token.split(':')
  <span class="missing-if-branch" title="if path not taken" >I</span>if (segments.length === 1) <span class="cstat-no" title="statement not covered" >throw new QiniuError(QiniuErrorName.InvalidToken, 'invalid token segments.')</span>
&nbsp;
  // token 构造的差异参考：https://github.com/qbox/product/blob/master/kodo/auths/UpToken.md#admin-uptoken-authorization
  const assessKey = segments.length &gt; 3 ? <span class="branch-0 cbranch-no" title="branch not covered" >segments[1] </span>: segments[0]
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!assessKey) <span class="cstat-no" title="statement not covered" >throw new QiniuError(QiniuErrorName.InvalidToken, 'missing assess key field.')</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    throw new QiniuError(QiniuErrorName.InvalidToken, 'token parse failed.')</span>
  }
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (putPolicy == null) {
<span class="cstat-no" title="statement not covered" >    throw new QiniuError(QiniuErrorName.InvalidToken, 'putPolicy is null.')</span>
  }
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (putPolicy.scope == null) {
<span class="cstat-no" title="statement not covered" >    throw new QiniuError(QiniuErrorName.InvalidToken, 'scope field is null.')</span>
  }
&nbsp;
  const bucketName = putPolicy.scope.split(':')[0]
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!bucketName) {
<span class="cstat-no" title="statement not covered" >    throw new QiniuError(QiniuErrorName.InvalidToken, 'resolve bucketName failed.')</span>
  }
&nbsp;
