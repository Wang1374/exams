...
<head>
    <title>Code coverage report for upload/base.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">upload</a> base.ts</h1>
        <div class='clearfix'>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>87/106</span>
            </div>
...
<a name='L338'></a><a href='#L338'>338</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">4x</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">4x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { QiniuErrorName, QiniuError, QiniuRequestError } from '../errors'
...
<span class="cstat-no" title="statement not covered" >      logger.error('get putPolicy from token failed.', error)</span>
<span class="cstat-no" title="statement not covered" >      this.onError(error)</span>
    }
  }
&nbsp;
  // 检查并更新 upload host
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (newHost == null) {
...
&nbsp;
<span class="fstat-no" title="function not covered" >  private h</span>andleError(error: QiniuError) {
<span class="cstat-no" title="statement not covered" >    this.logger.error(error.message)</span>
<span class="cstat-no" title="statement not covered" >    this.onError(error)</span>
  }
&nbsp;
  /**
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.file.size &gt; 10000 * GB) {
<span class="cstat-no" title="statement not covered" >      this.handleError(new QiniuError(</span>
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.putExtra.metadata) {
...
&nbsp;
    try {
      ...
      this.clear()
&nbsp;
      <span class="missing-if-branch" title="else path not taken" >E</span>if (err instanceof QiniuRequestError) {
        const reqId = this.aborted ? <span class="branch-0 cbranch-no" title="branch not covered" >'' </span>: err.reqId
        const code = this.aborted ? <span class="branch-0 cbranch-no" title="branch not covered" >-2 </span>: err.code
...
&nbsp;
  private clear() {
    ...
    this.xhrList.forEach(<span class="fstat-no" title="function not covered" >xhr </span>=&gt; {
