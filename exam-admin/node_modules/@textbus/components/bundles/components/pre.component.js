...
import { Injectable, BlockFormatter, Component, BackboneAbstractComponent, FormatData, FormatEffect, Fragment, InlineFormatter, Interceptor, TBSelection, VElement, BrComponent, VTextNode, DynamicKeymap, MarkdownSupport } from '@textbus/core';
...
class PreComponentLoader {
    ...
    read(el) {
        ...
        let code;
        if (lines.length) {
            code = Array.from(lines).map(i => i.innerText.replace(/[\s\n]+$/, '')).join('\n');
        }
        else {
...
class CodeFragment extends Fragment {
    constructor() {
        super(...arguments);
        this.blockCommentEnd = true;
        this.isBlockComment = false;
    }
    set blockCommentStart(b) {
...
let PreComponentInterceptor = class PreComponentInterceptor {
...
PreComponentInterceptor = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [TBSelection])
], PreComponentInterceptor);
let PreComponentDynamicKeymap = class PreComponentDynamicKeymap {
    constructor(selection) {
        this.selection = selection;
    }
    provide(instance) {
        return [{
                ...
                action: () => {
                    const firstRange = this.selection.firstRange;
                    const startIndex = instance.indexOf(firstRange.startFragment);
                    const endIndex = instance.indexOf(firstRange.endFragment);
...
PreComponentDynamicKeymap = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [TBSelection])
], PreComponentDynamicKeymap);
let PreComponentMarkdownSupport = class PreComponentMarkdownSupport {
    provide() {
        return {
            key: 'Enter',
            match(c) {
...
PreComponentMarkdownSupport = __decorate([
    Injectable()
], PreComponentMarkdownSupport);
let PreComponent = PreComponent_1 = class PreComponent extends BackboneAbstractComponent {
    ...
    clone() {
        ...
        const component = new PreComponent_1(this.lang, '');
        component.clean();
    ...
    render(isOutputMode, slotRendererFn) {
        ...
        block.attrs.set('theme', PreComponent_1.theme);
        return block;
    }
    setSourceCode(code) {
            ...
            if (languageGrammar) {
                ...
                if (nextSlot) {
                    if (typeof lastToken !== 'string' &&
                        ...
                        lastToken.content.indexOf(blockCommentStart) === 0) {
                        slot.blockCommentEnd = new RegExp(blockCommentEnd.replace(new RegExp(`[${blockCommentEnd}]`, 'g'), i => '\\' + i) + '$').test(lastToken.content);
...
PreComponent.theme = 'light';
PreComponent = PreComponent_1 = __decorate([
    Component({
...
export { PreComponent };
//# sourceMappingURL=pre.component.js.map