...
import { Injectable, Fragment, VElement, BackboneAbstractComponent, Component, Interceptor, TBSelection, BrComponent, I18n } from '@textbus/core';
class TableComponentLoader {
    constructor() {
        this.tagName = 'table';
    }
    match(component) {
...
let TableComponentInterceptor = class TableComponentInterceptor {
    constructor(selection, i18n) {
...
TableComponentInterceptor = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [TBSelection,
        I18n])
], TableComponentInterceptor);
let TableComponent = TableComponent_1 = class TableComponent extends BackboneAbstractComponent {
    constructor(config) {
        super('table');
        this.config = config;
    ...
    clone() {
        const clone = function (rows) {
            return rows.map(row => {
                return row.map(cell => {
                    return Object.assign(Object.assign({}, cell), { fragment: cell.fragment.clone() });
                });
            });
        };
        const config = {
        ...
        return new TableComponent_1(config);
    }
    componentDataChange() {
        this.clean();
        ...
        for (const row of this.config.bodies) {
            for (const col of row) {
                if (slot === col.fragment) {
                    const td = TableComponent_1.renderingCell(col);
    ...
    addColumnToLeft(selection) {
        ...
        cellMatrix.forEach(row => {
            ...
            if (index === 0) {
                cell.row.unshift(TableComponent_1.createCell());
            }
            else {
                if (cell.offsetColumn === 0) {
                    cell.row.splice(cell.row.indexOf(cell.cell), 0, TableComponent_1.createCell());
                }
    ...
    addColumnToRight(selection) {
        ...
        cellMatrix.forEach(row => {
            ...
            if (cell.offsetColumn + 1 < cell.cell.colspan) {
                ...
                cell.row.splice(cell.row.indexOf(cell.cell) + 1, 0, TableComponent_1.createCell());
            }
        });
        this.markAsDirtied();
    }
    addRowToTop(selection) {
        const selectRange = this.selectCells(selection);
        if (!selectRange) {
            return;
        }
        ...
        if (index === 0) {
            cellMatrix[0].cells.forEach(() => {
                tr.push(TableComponent_1.createCell());
            });
        }
        else {
    ...
    splitCells(selection) {
        ...
        cellMatrix.slice(minRow, maxRow + 1)
            ...
            if (cell.offsetRow !== 0 || cell.offsetColumn !== 0) {
                cell.cell.colspan = 1;
                cell.cell.rowspan = 1;
                const newCellFragment = TableComponent_1.createCell();
        ...
        prevRow.cellsPosition.forEach((cell, cellIndex) => {
            if (cell.offsetColumn === 0) {
                if (cell.offsetRow === 0 && cell.cell.rowspan > 1) {
                    const newCellFragment = TableComponent_1.createCell(cell.cell.rowspan - 1, cell.cell.colspan);
            ...
            if (cell.offsetRow === 0) {
                if (cell.offsetColumn > 0) {
                    ...
                    if (cell.cell.colspan > 1) {
                        const newCellFragment = TableComponent_1.createCell(cell.cell.rowspan, cell.cell.colspan - 1);
    ...
    selectCellsByRange(minRow, minColumn, maxRow, maxColumn) {
    ...
    findFragment(source) {
        ...
        while (source) {
            ...
            source = (_a = source.parentComponent) === null || _a === void 0 ? void 0 : _a.parentFragment;
        }
        return null;
    }
    normalize(startCell, endCell) {
    ...
    static createCell(rowspan = 1, colspan = 1) {
        const fragment = new Fragment();
