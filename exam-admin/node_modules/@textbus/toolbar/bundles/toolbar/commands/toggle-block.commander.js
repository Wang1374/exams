...
export class ToggleBlockCommander {
    constructor(tagName) {
    ...
    command(context) {
        ...
        selection.ranges.forEach(range => {
            if (overlap) {
                const context = range.commonAncestorFragment.getContext(BlockComponent, instance => {
                ...
                Array.from(contents.getFormatKeys()).forEach(token => {
                    contents.getFormatRanges(token).forEach(f => {
                        parentFragment.apply(token, Object.assign(Object.assign({}, f), { startIndex: f.startIndex + position, endIndex: f.endIndex + position }));
                    });
                });
            }
            else {
...
//# sourceMappingURL=toggle-block.commander.js.map