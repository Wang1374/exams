...
import { BranchAbstractComponent, Component, FormatData, FormatEffect, Fragment, VElement } from '@textbus/core';
import { BlockComponent } from '@textbus/components';
import { colorFormatter, fontSizeFormatter, boldFormatter } from '@textbus/formatters';
...
class TimelineComponentLoader {
    match(element) {
        return element.nodeName.toLowerCase() === 'tb-timeline';
    }
    read(element) {
        const slots = [];
        const list = Array.from(element.children).map(child => {
            let type = null;
...
let TimelineComponent = TimelineComponent_1 = class TimelineComponent extends BranchAbstractComponent {
    constructor(list) {
        super('tb-timeline');
        this.slots.push(...list);
    }
    clone() {
        return new TimelineComponent_1(this.slots.map(f => f.clone()));
    }
    ...
    renderItem(slot, isOutput) {
        const classes = ['tb-timeline-item'];
        if (slot.type) {
            classes.push('tb-timeline-item-' + slot.type);
        }
        const content = VElement.createElement("div", { class: "tb-timeline-content" });
        const child = (VElement.createElement("div", { class: classes.join(' ') },
            VElement.createElement("div", { class: "tb-timeline-line" }),
            VElement.createElement("div", { class: "tb-timeline-icon", title: isOutput ? null : '点击切换颜色', onClick: () => {
                    const currentType = slot.type;
                    if (!currentType) {
                        slot.type = timelineTypes[0];
                    }
                    else {
                        slot.type = timelineTypes[timelineTypes.indexOf(currentType) + 1] || null;
                    }
                    slot.markAsDirtied();
                } }),
            !isOutput && VElement.createElement("span", { class: "tb-timeline-add", onClick: () => {
...
export const timelineComponentExample = {
    name: i18n => i18n.get('components.timelineComponent.creator.name'),
    category: 'TextBus',
