...
var BaiduMapComponent_1;
import { Component } from '@textbus/core';
import { ImageComponent } from '@textbus/components';
class BaiduMapComponentLoader {
    match(element) {
        return element.nodeName.toLowerCase() === 'img' &&
            /^https:\/\/api\.map\.baidu\.com\/staticimage\/v2/.test(element.src);
    }
    read(element) {
        const component = new BaiduMapComponent(element.src, {
            width: (element === null || element === void 0 ? void 0 : element.style.width) || '',
            height: (element === null || element === void 0 ? void 0 : element.style.height) || ''
        });
        return {
            component: component,
            slotsMap: []
...
let BaiduMapComponent = BaiduMapComponent_1 = class BaiduMapComponent extends ImageComponent {
    clone() {
        return new BaiduMapComponent_1(this.src, {
            width: this.width,
            height: this.height
        });
    }
};
BaiduMapComponent.ak = '';
BaiduMapComponent = BaiduMapComponent_1 = __decorate([
    Component({
        loader: new BaiduMapComponentLoader()
    })
], BaiduMapComponent);
export { BaiduMapComponent };
export const baiduMapComponentExample = {
    name: i18n => i18n.get('components.baiduMapComponent.creator.name'),
    category: 'TextBus',
    ...
    factory(dialog, _, i18n) {
        const childI18n = i18n.getContext('components.baiduMapComponent.creator.form');
        const div = document.createElement('div');
        div.classList.add('textbus-form', 'textbus-form-ui');
...
<div style="height: 300px; position: relative">
  ...
  <form class="textbus-input-group" style="position: absolute; z-index: 1; left: 50px; top: 20px; width: 400px; box-shadow: 1px 2px 3px rgba(0,0,0,0.2); border-radius: 4px;">
    <input type="search" class="textbus-form-control textbus-input-block" placeholder="${childI18n.get('placeholder')}">
    <button type="submit" class="textbus-btn textbus-btn-primary">${childI18n.get('searchBtnText')}</button>
  </form>
</div>
<div class="textbus-btn-wrap" style="padding: 10px 20px; text-align: right; border-top: 1px solid #e9eaec; background: #fff">
<button type="button" class="textbus-btn textbus-btn-primary">${childI18n.get('confirmBtnText')}</button>
<button type="button" class="textbus-btn textbus-btn-default">${childI18n.get('cancelBtnText')}</button>
</div>
    `;
        const mapContainer = div.querySelector('.textbus-baidu-map-container');
        ...
        form.addEventListener('submit', ev => {
            ...
            myGeo.getPoint(input.value, function (newPoint) {
                point = newPoint;
                if (point) {
